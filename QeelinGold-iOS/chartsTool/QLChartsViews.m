...
+ (LineChartView *)getMyLineChartView{
   LineChartView *chartView = [[LineChartView alloc]init];
    ...
    chartView.dragDecelerationFrictionCoef = 0.01;//拖拽后惯性效果的摩擦系数(0~1)，数值越小，惯性越不明显
...
//    chartView.xAxis.xAxisIsEvenlyEnabled = YES;
//    [chartView.xAxis setLabelCount:4 force:YES];
...
//    chartView.leftAxis.labelPosition = YAxisLabelPositionInsideChart;
//    chartView.rightAxis.labelPosition = YAxisLabelPositionInsideChart;
    ...
    QLTimeLineMarker *marker = [QLChartsViews getMyTimeLineMarker];
...
//        comChartView.leftAxis.labelPosition = YAxisLabelPositionInsideChart;
    }else{
...
//    comChartView.legend.colorsObjc = colorArray;
    if (viewType == QLChartsViewsCandleStick) {
        [comChartView setViewPortOffsetsWithLeft:35 top:10 right:5 bottom:15];
        ...
        QLCandleStickMarker *marker = [QLChartsViews getMyCandleStickMarker];
        marker.chartView = comChartView;
        comChartView.marker = marker;
    }else if (viewType == QLChartsViewsBar){
        [comChartView setViewPortOffsetsWithLeft:35 top:10 right:5 bottom:10];
        [comChartView setAutoScaleMinMaxEnabled:YES];//匹配y轴数据自动适配
        ...
        comChartView.rightAxis.labelFont = [UIFont systemFontOfSize:0.f];
        comChartView.drawMarkers = YES;
    }
    comChartView.highlightPerDragEnabled = YES;
    comChartView.highlightPerTapEnabled = YES;
    return comChartView;
}
...
+ (QLTimeLineMarker *)getMyTimeLineMarker{
    QLTimeLineMarker *marker = [[QLTimeLineMarker alloc]
                                initWithColor:[UIColor blackColor]
                                font: [UIFont systemFontOfSize:12.0]
                                textColor: UIColor.whiteColor
                                insets: UIEdgeInsetsMake(0.0, 0.0, 0.0, 0.0)];
    marker.minimumSize = CGSizeMake(50.f, 30.f);
    return  marker;
}
+ (QLCandleStickMarker *)getMyCandleStickMarker{
    QLCandleStickMarker *marker = [[QLCandleStickMarker alloc]
                                   initWithColor:[UIColor blackColor]
                                   font: [UIFont systemFontOfSize:12.0]
                                   textColor: UIColor.whiteColor
                                   insets: UIEdgeInsetsMake(0.0, 0.0, 0.0, 0.0)];
